...
在 Colab T4 上重跑 `python -m nanovllm_omni.bench profile-detail` 后**完
全一致**：
...
sudo -n /usr/local/bin/ncu --target-processes all \
  ...
  ~/venvs/vllm-omni/bin/python -m nanovllm_omni.bench time \
    --model /home/mcig/minimind-3o --mimi /home/mcig/mimi \
...
`python -m nanovllm_omni.bench time --max-tokens 16 --prompts short_03 --runs 2 --warmup 1`，每个 kernel `--launch-count 3`。k3 在 T4 上名字是 `unrolled_elementwise_kernel`（不再带 `direct_copy_kernel_cuda` 子串），regex 改成 `.*unrolled_elementwise_kernel.*`。
