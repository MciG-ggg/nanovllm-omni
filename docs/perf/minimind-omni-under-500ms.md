...
>
> 优化栈已合并进 `main`（`d2ebe56 perf(stack): consolidate qkv_fusion + rms_norm + rope into attention.py`，含 per-instance 修复），落地在 `nanovllm_omni/models/minimind_omni/attention.py` + `bundle.py`。在本地 GPU 上运行 `python -m nanovllm_omni.bench time` 即可复现当前数字。历史 session 记录已归档到 `docs/perf/archive/`（gitignored，仅本地）。
...
# WSL 性能 bench
ssh mcigs-wsl "cd ~/nanovllm-omni && .venv/bin/python -m nanovllm_omni.bench time \
