"""Tests for nanovllm_omni.bench.bench_minimind (TK-012).
...
from nanovllm_omni.bench import bench_minimind
from nanovllm_omni.bench.bench_minimind import (
    CSV_COLUMNS,
    DEFAULT_N,
    FIXED_MAX_TOKENS,
    FIXED_PROMPT,
...
def test_main_cpu_path_writes_csv(tmp_path: Path) -> None:
    ...
    with mock.patch("nanovllm_omni.bench.bench_minimind.gpu_label", return_value="cpu"):
...
def test_main_gpu_path_uses_run_n(tmp_path: Path) -> None:
    ...
    with (
        ...
        mock.patch("nanovllm_omni.bench.bench_minimind.gpu_label", return_value="RTX 3050"),
    ):
