...
honors deploy.use_cuda_graph after the _thinker_stage fix (#66).

Before the fix, the stage constructed its own bundle without use_cuda_graph,
so the deploy yaml default (true) never reached served requests even though
the bench harness (which sets bundle.use_cuda_graph directly via OmniBase)
was fast. This script loads MinimindBundle via _thinker_stage on the 3050 and
asserts bundle.use_cuda_graph tracks deploy.use_cuda_graph for both
...
print(f"deploy.use_cuda_graph = {deploy.use_cuda_graph}")
...
if not captured:
    ...
    sys.exit(1)
b = captured[0]
print(f"bundle.use_cuda_graph = {b.use_cuda_graph}")
if b.use_cuda_graph is not True:
    print(f"FAIL: stage bundle use_cuda_graph={b.use_cuda_graph} (expected True)")
    sys.exit(1)
print("PASS: stage bundle honors deploy.use_cuda_graph=True")

# Explicit-off must propagate
deploy_off = DeployConfig(use_cuda_graph=False)
captured.clear()
try:
    th.create_bundle = fake_create_record  # type: ignore[attr-defined]
    ...
    th.create_bundle = orig_create  # type: ignore[attr-defined]
b2 = captured[0]
print(f"deploy.use_cuda_graph=False -> bundle.use_cuda_graph = {b2.use_cuda_graph}")
if b2.use_cuda_graph is not False:
    print(f"FAIL: explicit False not honored (got {b2.use_cuda_graph})")
    sys.exit(1)
